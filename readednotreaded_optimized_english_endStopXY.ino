...
long arraySize = 726; //cantidad de fijaciones
long n = 1;
long m = 0;
int sheetWidth = 1900;
...
void findZero() {//si llega primero en X no busca el cero en Y
  ...
  if ((temperatureLoopCounter > 2) && (flagFindZero == LOW)) {
    ...
    if ( readSheet == true) { //empieza a Burn
      ...
      if (flag == HIGH) {
        ...
        //chequear que no esté en el endStop, si está ahí que esté en 0
        //posY es la diferencia entre puntos
        //le digo q ya llegó al punto, que no siga de esa forma
        ...
        else if (posY < 0) {//hacia arriba, esto es para agregasr los límites
          //Serial.println(valEndY);
          //  if (valEndY < thresholdEndStopY) {
          //   posY = 0;
          //     counterY = posY;
          //Serial.print("tocó fin de carrera");
          //    }
          digitalWrite(pinDirY, LOW);
          if (counterY < (-posY)) {
            moveMotor(pinStepsY, speedXY);
...
void sequenceUp() {//agregar flag para que deje de hacerlo
...
void sheetSensing() {
  ...
  if (exitOfMark == true) {
    ...
    for (int i = 0; i < 50; i++) {
      moveMotor(pinStepsUp, spreedRoll);
    }
    exitOfMark = false;
  }
